...
export default {
  props: ['is-block', 'language'],
  mounted() {
    if (this.isBlock && this.language) {
      try {
        const result = hljs.highlight(this.language, this.$refs.block.textContent)
        this.$refs.block.innerHTML = result.value
      } catch (err) {
        // unknown language
        // console.log(err)
      }
